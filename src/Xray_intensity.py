...
def fit_func(x, C, A0,x0,d0,A1,x1,d1,A2,x2,d2,A3,x3,d3,A4,x4,d4,A5,x5,d5,
	...
	return	(C + A0 * np.exp(-0.5 * np.square((x-x0) / d0))
			+ A1 * np.exp(-0.5 * np.square((x-x0+26.8) / (d0/2) ))
			+ A2 * np.exp(-0.5 * np.square((x-x0+44.) / d2 ))
			...
			+ A9 * np.exp(-0.5 * np.square((x-x9) / 4.))
			)
len_fit = 10
